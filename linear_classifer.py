...
def softmax(predictions):
    ...
    if predictions.ndim == 1:
        ...
        new_predictions = predictions - max_value[:, np.newaxis]
    new_predictions = np.exp(new_predictions)
    sum_predictions = np.sum(new_predictions, axis=predictions.ndim-1)
    
    if predictions.ndim == 1:
        probabilities = new_predictions / sum_predictions
    else:
        probabilities = new_predictions / sum_predictions[:, np.newaxis]
...
def cross_entropy_loss(probs, target_index):
    ...
    if probs.ndim == 1:
        ...
        mask[(np.arange(0, probs.shape[0]), (target_index.T[0]))] = 1
    
    return -np.sum(mask * np.log(probs))
