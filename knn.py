...
class KNN:
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            nearest_indices = np.argsort(dists[i])[0:self.k]
            pred[i] = Counter(self.train_y[nearest_indices]).most_common(1)[0][0]
    ...
    def predict_labels_multiclass(self, dists):
        ...
        for i in range(num_test):
            # TODO: Implement choosing best class based on k
            # nearest training samples
            labels, counts = np.unique(self.train_y[np.argsort(dists[i])[:self.k]], return_counts=True)
            pred[i] = labels[counts.argmax()]
        return pred
